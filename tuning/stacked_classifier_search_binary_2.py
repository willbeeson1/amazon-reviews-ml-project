...
df_train["binary_label_cutoff_2"] = (df_train["overall"] > 2).astype(int)
...
label_col = "binary_label_cutoff_2"
...
base_lr = LogisticRegression(solver='saga', class_weight='balanced', max_iter=10000, random_state=42)
base_rf = RandomForestClassifier(class_weight='balanced_subsample', random_state=42, n_jobs=-1)
base_gb = GradientBoostingClassifier(validation_fraction=0.2, n_iter_no_change=20, tol=1e-4, random_state=42)

meta_lr = LogisticRegression(solver='lbfgs', class_weight='balanced', max_iter=10000, random_state=42)
...
param_dist = {
    "prep__text__ngram_range": [(1,2), (1,3)], # from (1,1), (1,2), (1,30
    "prep__text__max_features": [5000, 8000, 12000], # from 5000, 8000, 12000
    "prep__text__min_df": [3, 4, 5],
    "prep__text__max_df": [0.7, 0.75, 0.85],
    "prep__text__binary": [False, True],
    "prep__text__stop_words": [ None ],
    "prep__text__norm": [None, "l1", "l2"],
    "prep__text__use_idf": [False],
    "prep__text__sublinear_tf": [True, False],
    "prep__text__token_pattern": [r"\w{1,}"], # Removed None & unstable & r"\w{2,}", "(?u)\b\w\w+\b"

    # Custom features
    "prep__num__weight__sentiment_weight": [1.0, 1.5, 2.0, 3.0],

    # RandomForest
    "clf__rf__n_estimators": randint(50, 150),
    "clf__rf__max_depth": [5, 10, 15, None],
    # GradientBoosting
    "clf__gb__n_estimators": randint(30, 80),
    "clf__gb__learning_rate": loguniform(1e-2, 0.3),
    "clf__gb__max_depth": [3,4,5],
    # meta logistic
    "clf__final_estimator__C": loguniform(1e-3, 3),
    # base logistic
    "clf__lr__C": loguniform(1e-3, 0.3) 
}
...
print("\n=== Binary Classifier Cutoff [2] Tuning Start ===\n")
search.fit(X, y)

end_time = time.time()
print(f"\n=== Tuning Completed! Total Time Taken: {end_time - start_time:.2f} seconds (~{(end_time - start_time) / 60:.2f} min) ===\n")

print("\033[92m\n=== Binary Classifier Cutoff [2] Tuning Results ===\n\033[0m")

print("\033[92mBEST PARAMS:\n\033[0m")
for key, value in search.best_params_.items():
    print(f"  {key}: {value}")

print(f"\033[92mBEST CV Macro F1: {search.best_score_:.4f} \n\033[0m")
...
df_test["binary_split_2"] = test_preds
df_test["id"] = df_test.index

os.makedirs("results", exist_ok=True)
out_path = "results/submission_binary_split_2_from_tuner_v2.csv"
df_test[["id","binary_split_2"]].to_csv(out_path, index=False)
