# stacked_classifier_search_multiclass.py
...
X = df_train.drop(columns=["overall"])    # everything else as input
...
base_lr = LogisticRegression(solver='saga', class_weight='balanced', multi_class="multinomial", max_iter=10000)
...
param_dist = {
    ...
    "clf__gb__max_features": [0.7, "sqrt", "log2"],
...
# Changed to suit for multiclass
y_bin = label_binarize(y, classes=[1,2,3,4,5])  # shape (n_samples, 5)
y_proba_cv = cross_val_predict(best_model, X, y, 
                               cv=5, method="predict_proba")
# For each of the 5 classes:
for i in range(5):
    fpr, tpr, thresholds = roc_curve(y_bin[:, i], y_proba_cv[:, i])
    roc_auc = auc(fpr, tpr)
    print(f"Class {i+1} AUC:", roc_auc)
...
out_path = "results/submission_multiclass_from_tuner-v1.csv"
df_test[["id","predicted"]].to_csv(out_path, index=False)
