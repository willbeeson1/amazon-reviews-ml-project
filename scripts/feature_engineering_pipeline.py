...
###############################################################################
# 2. TIME FEATURES EXTRACTION (UPDATED)
###############################################################################

def process_time_features(df):
    """Extracts numerical time features from review timestamps."""
    if 'reviewTime' in df.columns:
        df['review_date'] = df['reviewTime'].apply(lambda x: 
            pd.to_datetime(x, format='%m %d, %Y', errors='coerce'))
    
    if 'unixReviewTime' in df.columns:
        mask = df.get('review_date', pd.Series()).isna()
        df.loc[mask, 'review_date'] = pd.to_datetime(
            df.loc[mask, 'unixReviewTime'], unit='s', errors='coerce')
    ...
    if 'review_date' in df.columns:
        ...
        df['review_dayofweek'] = df['review_date'].dt.dayofweek.fillna(0).astype(int)
        df['review_hour'] = df['review_date'].dt.hour.fillna(0).astype(int)  # NEW FEATURE
    
    return df

###############################################################################
# 3. TEXT STATISTICS & LEXICAL FEATURES
###############################################################################
...
###############################################################################
# 4. ADVANCED SENTIMENT ANALYSIS WITH NEGATION HANDLING
###############################################################################
...
###############################################################################
# 5. APPLY ALL PREPROCESSING STEPS
###############################################################################
